...
class Profile {
  ...
  Profile(
      ...
      required this.uid,
      required this.matchChangedTime,
      this.imageUrls,
      this.headline,
      this.description,
      this.age,
      ...
      required this.showUserGender,
      required this.relationshipType});
  Profile.fromJson(Map json, {DateTime? lastUpdateTime})
      : this.uid = json[API_CONSTS.USER_UID_KEY].toString(),
        this.hobbies = List<String>.from(
            jsonDecode(json[API_CONSTS.USER_HOBBIES] ?? jsonEncode([]))),
        this.pets = List<String>.from(
            jsonDecode(json[API_CONSTS.USER_PETS] ?? jsonEncode([]))),
        this.imageUrls = json[API_CONSTS.USER_IMAGES] != null
            ? List<String>.from(json[API_CONSTS.USER_IMAGES])
            : null,
        ...
        this.matchChangedTime = json[API_CONSTS.USER_MATCH_CHANGED_TIME] != null
            ? DateTime.fromMillisecondsSinceEpoch(
                    (json[API_CONSTS.USER_MATCH_CHANGED_TIME] * 1000).toInt())
                .toUtc()
            : null,
