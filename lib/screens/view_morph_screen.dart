...
class ViewMorphScreen extends StatefulWidget {
  ...
  final String celebImageUrl  = Get.arguments['celeb_image_url']; //celeb_image/Diego Boneta/wiki_image.jpeg
  final String userFaceUrl = Get.arguments['face_image_url']; //https://services.voilaserver.com/analyze-user-fr/fr_face_image/2BHZXM59SIb7vEfheLMIxY55zbN2/1656467868.7814832_2BHZXM59SIb7vEfheLMIxY55zbN2_96336.jpg/0
  @override
...
class _ViewMorphScreenState extends State<ViewMorphScreen> {
  ...
  Future<void> performMorphFaces()async{
    Tuple2<ServerResponse,String?> resultOfMorphing = await AWSServer.instance.morphFaces(celebUrl: widget.celebImageUrl,userUrl: widget.userFaceUrl);
  ...
  Widget _buildMorphinStatusWidget(){
    if(statusOfMorphing==MorphingStatus.morphing){
      return Padding(
        ...
        child: SimpleTimer(duration:Duration(milliseconds: 8500),status: TimerStatus.start,displayProgressText: false,),
      );
    }

    if(statusOfMorphing==MorphingStatus.error){
      return Text('An error has occured at server,sorry!',style: TextStyle(color: Colors.red),);
    }
